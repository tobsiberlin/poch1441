...
enum WallclockEvidenceRecorder {
    ...
    private static func draw(image: UIImage, into buffer: CVPixelBuffer) throws {
        ...
        // The pixel buffer is consumed row-first by AVAssetWriter. Applying the
        // UIKit-to-Core-Graphics flip here inverted the encoded MP4 even though
        // the pre-encode UIImage and contact sheet were upright.
    ...
    private static func outputDirectory() throws -> URL {
        ...
        return documents.appendingPathComponent("CardDealRhythmEvidenceV4", isDirectory: true)
    }
}
