...
enum WallclockEvidenceRecorder {
    ...
    static func captureAll(controller: CardMotionController) async throws {
        ...
        try writeContactSheet(
            ...
            to: output.appendingPathComponent("realtime-10-deal-overlap-sheet-402x874.png"),
            columns: 8,
            cellWidth: 134
        )
    ...
    private static func captureDeals(
        ...
        controller.startDealBurst(count: 10, interval: 0.135)
        let start = CACurrentMediaTime()
        var timedSnapshots: [TimedSnapshot] = []
        ...
        for index in 0..<58 {
            ...
            let file = "402x874-wallclock-10-deals-\(String(format: "%03d", index)).png"
            timedSnapshots.append(TimedSnapshot(
                snapshot: snapshot,
                record: WallclockFrameRecord(
                    file: file,
                    viewport: "402x874",
                    sequence: "10-deal-overlap",
                    frameIndex: index,
                    captureTime: CACurrentMediaTime(),
                    controllerTime: snapshot.wallclockTime,
            ...
            if snapshot.handCardCount == 10, snapshot.flights.isEmpty { break }
        }
        guard maximumOverlap >= 3, maximumOverlap <= 5 else {
    ...
    private static func outputDirectory() throws -> URL {
        ...
        return documents.appendingPathComponent("CardMotionEvidenceV3", isDirectory: true)
    }
}
