...
struct CardMotionLayout: Equatable, Codable, Sendable {
    ...
    init(width: Double, height: Double) {
        ...
        let cardWidth = min(max(shortEdge * 0.205, 64), 82)
        cardSize = MotionSize(width: cardWidth, height: cardWidth * 74 / 52)
        if width > height {
            deckCenter = MotionVector(x: width * 0.17, y: height * 0.40)
            handCenter = MotionVector(x: width * 0.78, y: height * 0.70)
            playCenter = MotionVector(x: width * 0.52, y: height * 0.43)
        } else {
            deckCenter = MotionVector(x: width * 0.21, y: height * 0.25)
            handCenter = MotionVector(x: width * 0.70, y: height * 0.73)
            playCenter = MotionVector(x: width * 0.52, y: height * 0.47)
        }
        deckYaw = -7
        handYaw = 8
        playYaw = -2
...
struct ProjectedShadow: Equatable, Codable, Sendable {
    ...
    static func project(
        ...
        return ProjectedShadow(
            points: points,
            blurRadius: 1.8 + 5.6 * heightFactor,
            opacity: 0.30 - 0.13 * heightFactor
        )
    }
}
...
struct ActiveCardFlight: Equatable, Sendable {
    ...
    func sample(at time: Double) -> CardFlightSnapshot {
        ...
        let restitution = contacted
            ...
            : 0
        let center = baseCenter - approach * (2.4 * restitution)
        let height = baseHeight + 2.8 * restitution
        let horizontalVelocity = contacted ? approach * (-18 * cos(.pi * settleProgress)) : baseVelocity
        let verticalVelocity = contacted ? 2.8 * (.pi * cos(.pi * settleProgress) * (1 - settleProgress) - sin(.pi * settleProgress)) / Self.settleDuration : baseVerticalVelocity
        ...
        let yaw = startYaw + (endYaw - startYaw) * smooth(progress) + (contacted ? 1.4 * restitution : 0)
...
struct CardMotionEngine: Equatable, Sendable {
    ...
    private(set) var handCardCount = 0
    private(set) var tableCardCount = 0
    ...
    @discardableResult
    mutating func startDeal(at time: Double, handOffset: MotionVector = .zero) -> Int? {
        ...
        let duration = max(0.38, min(0.52, 0.32 + delta.length / 1_450))
        let flight = ActiveCardFlight(
            ...
            path: Hermite2D(
                ...
                startVelocity: delta.normalized * min(390, delta.length / duration * 0.74),
                endVelocity: delta.normalized * min(170, delta.length / duration * 0.28),
                duration: duration
            ),
            heightPath: nil,
            arcHeight: min(max(layout.viewport.height * 0.105, 38), 78),
            startYaw: layout.deckYaw,
            endYaw: layout.handYaw + handOffset.x * 0.08,
    ...
    mutating func scheduleDealBurst(count: Int, interval: Double, at time: Double) {
        scheduledDeals = (0..<min(max(count, 0), deckLayerCount)).map { index in
            let fanIndex = Double(index) - Double(max(0, count - 1)) / 2
            return ScheduledDeal(
                startTime: time + Double(index) * interval,
                handSlotOffset: MotionVector(x: fanIndex * layout.cardSize.width * 0.12, y: abs(fanIndex) * 1.4)
            )
        }
    }
    ...
    mutating func setReducedMotion(_ enabled: Bool, at time: Double) {
        ...
        for flight in activeFlights {
            ...
            case .hand: handCardCount += 1
            case .table: tableCardCount += 1
            }
        }
    ...
    mutating func advance(to time: Double) -> StageSnapshot {
        ...
        while let first = scheduledDeals.first, first.startTime <= now {
            ...
            _ = startDeal(at: first.startTime, handOffset: first.handSlotOffset)
        }
