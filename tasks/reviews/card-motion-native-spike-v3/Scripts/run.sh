...
xcodebuild \
  -project CardMotionNativeSpikeV3.xcodeproj \
  -scheme CardMotionNativeSpikeV3 \
  -sdk iphonesimulator \
...
APP_PATH="$SPIKE_DIR/.derived/Build/Products/Debug-iphonesimulator/CardMotionNativeSpikeV3.app"
BUNDLE_ID="com.tobc.reviews.card-motion-native-spike-v3"
...
for _ in {1..240}; do
  if [[ -f "$DATA_CONTAINER/Documents/CardMotionEvidenceV3/export.complete" ]]; then
    break
  fi
  sleep 0.25
done
...
cp "$DATA_CONTAINER/Documents/CardMotionEvidenceV3/"* Evidence/
...
sips -g pixelWidth -g pixelHeight \
  Evidence/390x844-wallclock-play-cancel-return-000.png \
  Evidence/402x874-wallclock-play-cancel-return-000.png \
  Evidence/667x375-wallclock-play-cancel-return-000.png \
