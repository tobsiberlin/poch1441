(() => {
  ...
  function shadowTransform(value, lift = 0) {
    return `translate3d(${(value.x + lift * .22).toFixed(2)}px, ${(value.y + 7 + lift * .38).toFixed(2)}px, 0) `
  ...
  function flightDuration(from, to, kind, index = 2) {
    ...
    diagnostics.durations.push({ kind, index, distance: Number(distance.toFixed(2)), duration });
    return duration;
  }
  ...
  async function dealCard(card, index, currentLayout, generation, reduced) {
    await wait(index * (reduced ? 32 : 72), generation);
    if (generation !== state.generation) return;
    const start = deckStart(currentLayout, index);
    ...
    const midpoint = mixPose(start, end.outer, .54, 34 + Math.abs(index - 2) * 3);
    ...
    await Promise.all([
      ...
      animateElement(card.shadow, [
        ...
        { opacity: .1, transform: shadowTransform(midpoint, 34), offset: .54 },
  ...
  async function flipCard(card, currentLayout, generation, reduced, faceUp) {
    ...
    await Promise.all([
      ...
      animateElement(card.shadow, [
        ...
        { opacity: .19, transform: shadowTransform(lifted, 13), offset: .48 },
        { opacity: .42, transform: shadowTransform(target.outer) },
      ], { duration: 420, easing: EASE_IN_OUT }, generation),
    ]);
  }
  ...
  async function movePlayed(card, currentLayout, generation, reduced, outward) {
    ...
    const midpoint = mixPose(start.outer, end.outer, .52, outward ? 22 : 18);
    ...
    await Promise.all([
      ...
      animateElement(card.shadow, [
        ...
        { opacity: .12, transform: shadowTransform(midpoint, outward ? 22 : 18), offset: .5 },
    ...
    if (!outward && generation === state.generation) {
      card.element.style.zIndex = "22";
      card.shadow.style.zIndex = "12";
    }
  }
  ...
  async function runFrom(startIndex = 0) {
    ...
    if (startIndex <= 0) {
      setPhase(0);
      await Promise.all(cards.map((card, index) => dealCard(card, index, currentLayout, generation, reduced)));
      if (generation !== state.generation) return;
    }
