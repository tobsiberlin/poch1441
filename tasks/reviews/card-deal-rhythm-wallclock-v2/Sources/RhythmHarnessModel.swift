...
enum HarnessCardPhase: String, Codable, Sendable {
    ...
    var isMoving: Bool {
        switch self {
        case .travel, .contact, .settle, .cancelling: true
        case .rest: false
        }
    }
}
...
struct RhythmHarnessPlan: Sendable {
    ...
    static let cancelClearSeconds = 0.14
    ...
    static func make(sequence: HarnessSequenceKind) throws -> RhythmHarnessPlan {
        ...
        let end: Double
        if let cancellation {
            end = cancellation + cancelClearSeconds + 0.28
        } else {
    ...
    func snapshot(at elapsedSeconds: Double) -> HarnessSnapshot {
        ...
        for index in schedule.indices {
            ...
            if let cancellationSeconds, elapsed >= cancellationSeconds {
                if card.actualStartSeconds >= cancellationSeconds { continue }
                if card.restWindowStartSeconds <= cancellationSeconds {
                    cards.append(HarnessCardSnapshot(index: index, phase: .rest, phaseProgress: 1))
                    continue
                }
                let cancellationProgress = min(
                    max((elapsed - cancellationSeconds) / Self.cancelClearSeconds, 0),
                    1
                )
                if cancellationProgress < 1 {
                    cards.append(HarnessCardSnapshot(
                        index: index,
                        phase: .cancelling,
                        phaseProgress: cancellationProgress
                    ))
                }
                continue
            }
        ...
        let effectiveTime = min(elapsed, cancellationSeconds ?? elapsed)
        let startedCount = schedule.filter { $0.actualStartSeconds <= effectiveTime }.count
