...
xcodebuild \
  -project CardDealRhythmWallclockV2.xcodeproj \
  -scheme CardDealRhythmWallclockV2 \
  -sdk iphonesimulator \
...
APP_PATH="$HARNESS_DIR/.derived/Build/Products/Debug-iphonesimulator/CardDealRhythmWallclockV2.app"
BUNDLE_ID="com.tobc.reviews.card-deal-rhythm-wallclock-v2"
...
for _ in {1..720}; do
  if [[ -f "$DATA_CONTAINER/Documents/CardDealRhythmEvidenceV2/export.complete" ]]; then
    break
  fi
  sleep 0.25
done
...
cp "$DATA_CONTAINER/Documents/CardDealRhythmEvidenceV2/"* Evidence/
VIDEO_INSPECTOR_PATH=/tmp/poch1441-card-deal-rhythm-video-inspector
xcrun swiftc \
  -warnings-as-errors \
  -parse-as-library \
