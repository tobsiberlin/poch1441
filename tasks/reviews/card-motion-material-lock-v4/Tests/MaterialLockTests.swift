...
import UIKit
import CryptoKit
@testable import CardMotionMaterialLockV4

final class MaterialLockTests: XCTestCase {
    func testFrozenV1V2V3GeometryTimingContactAndMotionHashesMatch() throws {
        let v4Root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let reviewsRoot = v4Root.deletingLastPathComponent()
        for revision in [
        ...
        let v3Root = reviewsRoot.appendingPathComponent(FrozenMotionContract.v3.reference)
        for file in [
            "MaterialLockModel.swift",
            "MaterialLockController.swift",
            "MaterialLockStage.swift",
            "W2CardMaterialContract.swift",
        ] {
            ...
            XCTAssertEqual(v4Data, v3Data, "\(file) drifted; V4 is material-only")
        }
        let w2Data = try Data(contentsOf: v4Root.appendingPathComponent("Sources/W2CardMaterialContract.swift"))
    ...
    func testV4RemovesOvalPatternAndPreservesFrozenV3Presentation() throws {
        let root = URL(fileURLWithPath: #filePath).deletingLastPathComponent().deletingLastPathComponent()
        let material = try String(
            contentsOf: root.appendingPathComponent("Sources/ProductW2CardSurface.swift"),
            encoding: .utf8
